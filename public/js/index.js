(function() {
	...
	$(document).swipeUp(function() {
		...
		pageMove(current.next(), current, 'up');
	})

	$(document).swipeDown(function() {
		if (isAnimating) return;
		var current = $('.page-current');
		pageMove(current.prev(), current, 'down');
	})

	function pageMove(now, last, dis) {
		var nowPage = now;
		var lastPage = last;
		if (!nowPage.length) {
			return
		...
		setTimeout(function() {
			...
			$(lastPage).find("img,p").addClass("hide");
			$(nowPage).addClass('page-current');
			$(nowPage).removeClass(inClass);
			// $(nowPage).find("img").removeClass("hide");
			$(nowPage).find("img,p,div").removeClass("hide");
			isAnimating = false;
		}, 600);
	}
})();
